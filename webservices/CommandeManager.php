...
if ($mode == "listCommande") {
    ...
    // Vérification et initialisation de $LG_CLIID
    if (!isset($LG_CLIID) || empty($LG_CLIID)) {
        $LG_CLIID = ""; // Assurer qu'une valeur valide est attribuée
    }

    $response = $CommandeManager->showAllOrOneCommandeproduit($LG_CLIID, $LG_COMMID, $token);

    // Vérification que $response contient bien des données 
    if (!isset($response->lines) || empty($response->lines)) {
        error_log("Aucune ligne de commande trouvée.");
    ...
    foreach ($response->lines as $key => $line) {
        ...
        // Vérification si $OProduit est un tableau et contient 'str_propic'
        if (is_array($OProduit) && isset($OProduit['str_propic']) && isset($OProduit['lg_proid'])) {
            $line->str_propic = Parameters::$rootFolderRelative . "produits/" . $OProduit['lg_proid'] . "/" . $OProduit['str_propic'];
        } else {
            $line->str_propic = null;
        }
    ...
    // Vérification avant affichage du JSON
    if (empty($OJson)) {
        error_log("Aucune donnée de produit trouvée.");
...
=======
    ...
    $arrayJson = array(
        ...
        "code_statut" => "1", // Même code statut que la version en ligne
        "desc_statut" => "Calendrier de livraison récupéré avec succès" // Même description
    );
...
>>>>>>> 4e428fc (modif)
    ...
    if ($mode == "getTypetransaction") {
        ...
        $arrayJson['data'] = [
            ...
            'str_socadresse' => isset($result['str_socadresse']) ? $result['str_socadresse'] : null,
        ...
        $FILTERS_OPTIONS = [];
            $result = $CommandeManager->getDeliveryPlace($FILTERS_OPTIONS, $LIMIT, $PAGE);
